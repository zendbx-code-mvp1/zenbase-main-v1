...
class ReverseProxyService:
    ...
    def generate_project_config(self, project) -> Path:
        ...
        Expects the project object to have:
            ...
            project.internal_port  str | int   (port the app listens on inside the container)
            project.port           str | int   (host-mapped port, fallback only)
            project.name           str

        Returns the path of the written config file.
        Raises ValueError if the project has no port assigned.
        """
        # Prefer internal_port (container-to-container), fall back to host port
        internal_port = getattr(project, "internal_port", None) or project.port
    ...
    def _build_server_block(self, subdomain: str, port: int, project_name: str) -> str:
        ...
        All project containers share the same Docker network as Nginx
        (zenbase-net), so we proxy by container hostname.

        We use Docker's embedded DNS resolver (127.0.0.11) and a $upstream
        variable so Nginx resolves the hostname at request time rather than
        at startup — this prevents Nginx from crashing if the container is
        temporarily down or not yet started.
        """
        container_host = subdomain  # Container name matches subdomain slug
...
server {{
    ...
    # Use Docker's embedded DNS so hostnames resolve at request time,
    # not at startup — prevents nginx from failing if container is down.
    resolver 127.0.0.11 valid=10s ipv6=off;
    set $upstream http://{container_host}:{port};
